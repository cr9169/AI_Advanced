...
import { isRoute, type Route } from "../../domain/types.js";
import { searchKnowledgeSources } from "../../infrastructure/knowledge/source.js";
...
async function loadGolden(): Promise<GoldenCase[]> {
  const file = path.join(REPO_ROOT, "evals/golden.json");
...
export async function runOfflineEvals(): Promise<boolean> {
  ...
  for (const item of cases) {
    ...
    if (item.expectedRoute === "technical") {
      const hits = await searchKnowledgeSources(item.query);
      sourceOk = hits
        .slice(0, 3)
