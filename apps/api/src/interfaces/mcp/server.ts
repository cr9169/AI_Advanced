import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import { ReadDocumentArgs, SearchKnowledgeArgs } from "../../application/tools/schemas.js";
import { formatError } from "../../shared/utils.js";
import {
  readKnowledgeSource,
  searchKnowledgeSources,
} from "../../infrastructure/knowledge/source.js";
import { GetSystemMetricsInput } from "./schemas.js";
import { dummyMetrics, textResult } from "./utils.js";

function createKnowledgeServer(): McpServer {
  const server = new McpServer({
    name: "knowledge-agent",
    version: "1.0.0",
  });

  server.registerTool(
    "get_system_metrics",
    {
      description:
        "Return dummy CPU, memory, disk, uptime, and latency metrics for a host.",
      inputSchema: GetSystemMetricsInput,
    },
    async ({ host }) => {
      try {
        return textResult(JSON.stringify(dummyMetrics(host ?? "localhost"), null, 2));
      } catch (error: unknown) {
        return textResult(
          `get_system_metrics failed: ${formatError(error)}`,
          true,
        );
      }
    },
  );

  server.registerTool(
    "read_document",
    {
      description:
        "Read a markdown/text file from knowledge (local disk or S3). Path is relative, e.g. rag.md.",
      inputSchema: ReadDocumentArgs,
    },
    async ({ path }) => {
      try {
        return textResult(await readKnowledgeSource(path));
      } catch (error: unknown) {
        return textResult(`read_document failed: ${formatError(error)}`, true);
      }
    },
  );

  server.registerTool(
    "search_knowledge",
    {
      description:
        "Deterministic keyword search over knowledge files (local disk or S3; no embeddings).",
      inputSchema: SearchKnowledgeArgs,
    },
    async ({ query }) => {
      try {
        const hits = await searchKnowledgeSources(query);
        return textResult(
          hits.length === 0
            ? "No knowledge files matched."
            : JSON.stringify(hits, null, 2),
        );
      } catch (error: unknown) {
        return textResult(
          `search_knowledge failed: ${formatError(error)}`,
          true,
        );
      }
    },
  );

  return server;
}

async function main(): Promise<void> {
  const server = createKnowledgeServer();
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error("knowledge-agent MCP server running on stdio");
}

main().catch((error: unknown) => {
  console.error(`MCP server failed: ${formatError(error)}`);
  process.exit(1);
});
